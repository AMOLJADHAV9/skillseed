...
/**
 * SplashButton Component
 * 3D Animated Splash Action Button with animated text letters, floating particles, and outline glow.
 *
 * @param {string} label1 - Primary label shown (default: 'Join Today')
 * @param {string} label2 - Secondary label shown on click/focus (default: 'Join Now')
 * @param {function} onClick - Click handler
 * @param {string} href - Optional URL link. Renders an <a> element wrapper if provided.
...
export const SplashButton = ({
  ...
  const renderAnimatedLetters = (text) => {
    return text.split('').map((char, index) => {
      ...
      return (
        <span key={index} data-label={displayChar} style={{ '--i': index + 1 }}>
          {displayChar}
        </span>
      );
    });
  ...
  return (
    <Component
      className={`button splash-button ${className}`}
      href={href}
      onClick={onClick}
      {...props}
    >
